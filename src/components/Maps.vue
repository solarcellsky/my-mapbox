...
class Custom3DControl {
  onAdd(map) {
    ...
    this.container.className =
      "mapboxgl-ctrl mapboxgl-ctrl-group custom-control";
...
export default {
  ...
  data() {
    return {
      accessToken:
        "pk.eyJ1Ijoic29sYXJjZWxsc2t5IiwiYSI6ImNrbmFmOXVjdjExZ3oycnRhY3ZqNWJqNHcifQ.INN_v0VVzC2KNQVL-nvMLg", // your access token. Needed if you using Mapbox
      mapStyle: "mapbox://styles/mapbox/dark-v10", // your map style
      drawer: false,
      coordinates: [[108.986133, 34.275893]],
      minZoom: 10,
      backgroundAlpha: 0,
      object: {},
      models: [
  ...
  methods: {
    ...
    initMap() {
      ...
      map.on("style.load", () => {
        this.addThreeDModel(map, this.models);
      });
    },
    ...
    addThreeDModel(map, options) {
      ...
      options.map(function (option) {
        if (option.extrusion) {
          self.createExtrusionLayer(
            map,
            option.geojson,
            option.sourceId + "-source",
            option.id + "-extrusion"
          );
        }
      });
